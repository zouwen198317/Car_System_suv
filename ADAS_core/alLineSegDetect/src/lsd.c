...
#ifndef WIN32
  Cache_wbInv((Ptr)range_l_s, n_bins*sizeof(struct coorlist *), Cache_Type_ALL, TRUE);
#endif
...
#ifndef WIN32
  Cache_wbInv((Ptr)list, n*p*sizeof(struct coorlist), Cache_Type_ALL, TRUE);
  Cache_wbInv((Ptr)range_l_s, n_bins*sizeof(struct coorlist *), Cache_Type_ALL, TRUE);
  Cache_wbInv((Ptr)range_l_e, n_bins*sizeof(struct coorlist *), Cache_Type_ALL, TRUE);
#endif
...
{
  static double inv[TABSIZE] = {0};   /* table to keep computed inverse values */
  double tolerance = 0.1;       /* an error of 10% in the result is accepted */
  double log1term = 0, term = 0, bin_term = 0, mult_term = 0, bin_tail = 0, err = 0, p_term = 0, eTmp = 0;
  int i = 0;
...
static double rect_nfa(struct rect * rec, image_double angles, double logNT)
{
  ...
  ri_del(i); /* delete iterator */
#if EAGLE_DEBUG // for debug
  eDebugTest = nfa(132, 119, rec->p, logNT);
#endif
...
#ifndef WIN32
	Cache_wbInv((Ptr)used->data, used->xsize * used->ysize, Cache_Type_ALL, TRUE);
	Cache_wbInv((Ptr)reg, (*reg_size)*sizeof(struct point), Cache_Type_ALL, TRUE);
#endif
                               ...
                               int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  int lListNum = 0;
#if EAGLE_DEBUG
  struct coorlist *list_test =NULL;
  FILE *pFileSrc = NULL;
  int lX = 0, lY = 0;
#endif
...
#if EAGLE_DEBUG>1 // for debug
  pFileSrc = fopen("Img_FLAT64.hex", "wb");
  fwrite(img, X * Y * sizeof(double), 1, pFileSrc);
  fclose(pFileSrc);
#endif
...
#ifndef WIN32
    Cache_wbInv((Ptr)scaled_image->data, scaled_image->xsize * scaled_image->ysize*sizeof(double), Cache_Type_ALL, TRUE);
#endif
#if EAGLE_DEBUG>1
      pFileSrc = fopen("gaussian.txt", "w");
      for (lX = 0; lX < scaled_image->xsize * scaled_image->ysize; lX++)
      {
          fprintf(pFileSrc, "%.3f\n", scaled_image->data[lX]);
      }
      fclose(pFileSrc);
#endif
      angles = ll_angle(scaled_image, rho, &list_p, &mem_p, &modgrad, (unsigned int) n_bins );
#ifndef WIN32
    Cache_wbInv((Ptr)angles->data, angles->xsize * angles->ysize*sizeof(double), Cache_Type_ALL, TRUE);
	Cache_wbInv((Ptr)modgrad->data, modgrad->xsize * modgrad->ysize*sizeof(double), Cache_Type_ALL, TRUE);
#endif
...
#if EAGLE_DEBUG
  pFileSrc = fopen("LIST.txt", "w");
  for (list_test = list_p; list_test != NULL; list_test = list_test->next)
  {
      lListCnt++;
  }

  fprintf(pFileSrc, "ListCnt = %d\n", lListCnt);
  for (list_test = list_p; list_test != NULL; list_test = list_test->next)
  {
      fprintf(pFileSrc, "%d %d\n", list_test->x, list_test->y);
  }
  fclose(pFileSrc);
  lListCnt = 0;
#endif
...
#if EAGLE_DEBUG > 1
  pFileSrc = fopen("angles.txt", "w");
  for (lX = 0; lX < angles->xsize * angles->ysize; lX++)
  {
      fprintf(pFileSrc, "%.3f\n", angles->data[lX]);
  }
  fclose(pFileSrc);

  pFileSrc = fopen("modgrad.txt", "w");
  for (lX = 0; lX < modgrad->xsize * modgrad->ysize; lX++)
  {
      fprintf(pFileSrc, "%.3f\n", modgrad->data[lX]);
  }
  fclose(pFileSrc);
  ...
  */
...
#if EAGLE_DEBUG>1
  pFileSrc = fopen("used.txt", "w");
  for (lX = 0; lX < used->xsize * used->ysize; lX++)
  {
      fprintf(pFileSrc, "%d\n", used->data[lX]);
  }
  fclose(pFileSrc);
  ...
  for (; list_p != NULL; list_p = list_p->next)
  {
      ...
      /* there is no risk of double comparison problems here, because we are only interested in the exact NOTDEF value */
...
#ifndef WIN32
    Cache_wbInv((Ptr)used->data, used->xsize * used->ysize, Cache_Type_ALL, TRUE);
	Cache_wbInv((Ptr)reg, xsize*ysize*sizeof(struct point), Cache_Type_ALL, TRUE);
#endif
          ...
          /* Check if the rectangle exceeds the minimal density of
             ...
             */
          lFlag = refine(reg, &reg_size, modgrad, reg_angle, prec, p, &rec, used, angles, density_th);
#ifndef WIN32
          Cache_wbInv((Ptr)used->data, used->xsize * used->ysize, Cache_Type_ALL, TRUE);
          Cache_wbInv((Ptr)reg, xsize*ysize*sizeof(struct point), Cache_Type_ALL, TRUE);
#endif
...
#if CHECK_NFA // this will go wrong in TI FW
          if (log_nfa <= log_eps) continue;
#endif

#if EAGLE_DEBUG
          printf("No.%d (%d , %d)Pass~~\n", lListCnt, list_p->x, list_p->y);
#endif
...
#if EAGLE_DEBUG
  printf("lListCnt = %d\n", lListCnt);
  pFileSrc = fopen("used_done.txt", "w");
  for (lX = 0; lX < used->xsize * used->ysize; lX++)
  {
      fprintf(pFileSrc, "%d\n", used->data[lX]);
  }
  ...
  *n_out = (int) (out->size);
  ...
  free( (void *) out );  /* only the 'ntuple_list' structure must be freed,
                            ...
                            as a result. */
#if EAGLE_DEBUG // for debug
  pFileSrc = fopen("Line.txt", "w+");
  for (lX = 0; lX < *n_out; lX++)
  {
      fprintf(pFileSrc, "Sx:%.1f Sy:%.1f Ex:%.1f Ey:%.1f\n", return_value[7 * lX], return_value[7 * lX + 1], return_value[7 * lX + 2], return_value[7 * lX + 3]);
  }
  fclose(pFileSrc);
#endif
  return return_value;
...
{
  /* LSD parameters */
  double sigma_scale = 1.0; // 0.6; /* Sigma for Gaussian filter is computed as sigma = sigma_scale/scale.                    */
  ...
  double log_eps = 0.0;// 0.0;     /* Detection threshold: -log10(NFA) > log_eps     */
