...
{           
    ...
    {                
        if (0 != blk_idx) {
            ...
            DAT_copy2d_lut(DAT_1D2D, pDMA_dst,
                       dst_buf + (p_area->start_row * img_width + p_area->start_col)*2,
                       p_area->width, p_area->height, img_width, 2);
...
{           
    ...
    for (i=0; i<pBInfo->ch_num; i++) {
        ...
        DAT_copy2d(DAT_2D1D, 
                   ...
                   pDMA_src + DMA_src_size,
                   pch->width, pch->height, a_uwImageWidth, 2);         
    ...
    {                
        if (0 != blk_idx) {
            ...
            DAT_copy2d(DAT_1D2D, pDMA_dst,
                       a_pucDestBuffer + ((p_area->start_row * a_uwImageWidth + p_area->start_col) << 1),
                       p_area->width, p_area->height, a_uwImageWidth, 2);
